...
int main(int argc, char *argv[])
{
    ...
    MPI_Status g_status[numworkers];
    MPI_Request g_reqs[numworkers];
    ...
    *********************************************************************************************************/
    ...
    int workers_with_no_extra_rows = numtasks - workers_with_no_extra_rows;
    ...
    for (int steps = 0; steps < numSteps; steps++)
    {
        ...
        if (bottom != -1)
        {
            MPI_Isend(&Tn[offset + (rows-1)*ny], ny, MPI_FLOAT, bottom, RIGHTREQUEST, MPI_COMM_WORLD, &b_reqs[0]);
            MPI_Irecv(&Tn[offset + rows*ny], ny, MPI_FLOAT, bottom, LEFTREQUEST, MPI_COMM_WORLD, &b_reqs[1]);
        }

        if(top != -1)
        {
            MPI_Waitall(2, t_reqs, status);
        }

        if (bottom != -1)
        {
            MPI_Waitall(2, b_reqs, status);
        }
        ...
        if ((steps+1) % outputEvery == 0){
            if(taskid == 0){
                ...
                for (int i = 1; i <= numworkers; i++) {
                    ...
                    MPI_Irecv(&Tn[g_offset], g_rows * ny, MPI_FLOAT, i, DONE, MPI_COMM_WORLD, &g_reqs[i-1]);
                }
                MPI_Waitall(numworkers, g_reqs, g_status);
                writeTemp(Tn, nx, ny, steps+1);
            }
            if(taskid != 0) {
                /**Send data to worker 0**/
                MPI_Isend(&offset, 1, MPI_INT, 0, DONE, MPI_COMM_WORLD, &output_reqs[0]);
                MPI_Isend(&rows, 1, MPI_INT, 0, DONE, MPI_COMM_WORLD, &output_reqs[1]);
                MPI_Isend(&Tn[offset], rows * ny, MPI_FLOAT, 0, DONE, MPI_COMM_WORLD, &output_reqs[2]);
